# Ventri Landing Page (Standalone) — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a fully static pre-launch waitlist landing page for Ventri as a standalone project at `/home/oz/Projects/Personal/ventri-landing/`, with client-side Supabase integration and EN/ES language switching.

**Architecture:** Standalone Astro 4 project with `output: 'static'`. No SSR, no server adapter. `EmailForm` submits directly to Supabase via the anon key + RLS (`anon insert only`). EN/ES i18n is client-side: `translations.ts` holds all strings; `i18n-client.ts` reads `localStorage`, stamps `[data-i18n]` elements, and wires the toggle button.

**Tech Stack:** Astro 4, Tailwind v4 (`@tailwindcss/vite`), `@supabase/supabase-js` (browser, anon key), vanilla TypeScript for carousel/accordion/i18n.
...
| File | Responsibility |
|---|---|
| `astro.config.mjs` | Astro config: static output + Tailwind v4 Vite plugin |
| `src/styles/global.css` | Design tokens + `@import "tailwindcss"` |
| `src/i18n/translations.ts` | All EN + ES UI strings |
| `src/i18n/i18n-client.ts` | Client runtime: localStorage → stamp `[data-i18n]` elements |
| `src/components/Navbar.astro` | Sticky nav: logo, lang toggle button, "Get notified" anchor |
| `src/components/EmailForm.astro` | Form: email + WhatsApp inputs, news toggle, Supabase submit |
| `src/components/Hero.astro` | Badge, H1, subline, `<EmailForm id="hero-form" />` |
| `src/components/Problem.astro` | 3 pain cards on dark bg |
| `src/components/HowItWorks.astro` | 3 numbered steps |
| `src/components/Features.astro` | 6-card feature grid |
| `src/components/Screenshots.astro` | Carousel with dot nav (vanilla JS) |
| `src/components/Faq.astro` | 4-item accordion (`<details>`) |
| `src/components/FooterCta.astro` | CTA copy + `<EmailForm id="footer-form" dark />` |
| `src/components/Footer.astro` | Copyright bar |
| `src/pages/index.astro` | Root page: imports all components, mounts i18n script |
| `.env.example` | Placeholder env var docs (committed) |
...
**Files:**
- Create: `/home/oz/Projects/Personal/ventri-landing/` (entire project)
...
```bash
cd /home/oz/Projects/Personal
npm create astro@latest ventri-landing -- --template minimal --no-install --no-git --yes
cd ventri-landing
npm install
```

If `--yes` prompts anyway, select: TypeScript → strict, install deps → no, git → no.

Expected: `ventri-landing/` created, `node_modules` installed, no errors.

- [ ] **Step 2: Install Tailwind v4 + Supabase**

```bash
npm install tailwindcss @tailwindcss/vite @supabase/supabase-js
```

Expected: packages in `package.json`, no peer-dependency warnings.

- [ ] **Step 3: Init git, verify `.gitignore` includes `.env`**

```bash
git init
```

Open `.gitignore` (Astro scaffold creates one). Confirm these lines exist — add any that are missing:
...
Expected: Astro dev server at `http://localhost:4321`, no errors in terminal.

- [ ] **Step 7: Commit**

```bash
git add .
git commit -m "feat: scaffold Astro + Tailwind v4 project"
```

---

## Task 2: Supabase table + env files
...
Expected: table visible in Supabase Table Editor, RLS enabled, one policy listed.
...
Replace placeholders with values from Supabase → Settings → API. The anon key (not service role key) goes here.
...
git commit -m "feat: add EN/ES translations"
```

---

## Task 4: i18n client runtime

**Files:**
- Create: `src/i18n/i18n-client.ts`
...
```bash
git add src/i18n/i18n-client.ts
git commit -m "feat: add client-side i18n runtime"
```
...
- [ ] **Step 2: Replace `src/pages/index.astro` with a shell that mounts Navbar**
...
- "Get notified" and "ES" button visible on right
- Clicking "ES" changes button text to "EN"
- Refreshing with ES active keeps "EN" button (localStorage persists)
...
**Files:**
- Create: `src/components/EmailForm.astro`

The form submits directly to Supabase using the anon key. No server route.

- [ ] **Step 1: Create `src/components/EmailForm.astro`**
...
<div id={id} class="w-full max-w-md" data-form-wrapper>
  <div class="flex flex-col gap-2" data-fields>
    <input
      type="email"
      name="email"
  ...
  <label class="flex items-center gap-2 mt-3 cursor-pointer select-none" data-news-label>
    <input
      type="checkbox"
      name="subscribeNews"
...
<script>
  ...
  document.querySelectorAll<HTMLElement>('[data-form-wrapper]').forEach((wrapper) => {
    ...
    const fieldsEl = wrapper.querySelector<HTMLElement>('[data-fields]')!;
    const newsLabel = wrapper.querySelector<HTMLElement>('[data-news-label]')!;
    ...
    submitBtn.addEventListener('click', async () => {
      ...
      newsLabel.classList.add('hidden');
...
- [ ] **Step 2: Add EmailForm to `src/pages/index.astro` temporarily for isolated testing**
...
<div style="padding: 2rem;">
  <EmailForm id="test-form" />
</div>
```

- [ ] **Step 3: Test form in browser** (`.env` must be populated with real Supabase values)

- Submit empty → "Enter your email or WhatsApp number"
- Submit invalid email → "Enter a valid email"
- Submit invalid WhatsApp (`123`) → "Enter a valid WhatsApp number"
- Submit valid email only → success state + row in Supabase with `whatsapp = null`
- Submit valid WhatsApp only (`+15551234567`) → success state + row with `email = null`
- Submit same email again → success state shown (23505 treated as success)
- Check news toggle on → Supabase row has `subscribe_news = true`
- Switch lang to ES, trigger error → messages appear in Spanish

- [ ] **Step 4: Remove test scaffolding from `index.astro`** (revert the `import EmailForm` and `<div>` added in Step 2)

- [ ] **Step 5: Commit**

```bash
git add src/components/EmailForm.astro src/pages/index.astro
git commit -m "feat: add EmailForm with Supabase client-side submit"
```

---

## Task 7: Hero, Problem, HowItWorks
...
<section id="top" class="py-20 px-4 bg-[var(--bg-primary)]">
  <div class="max-w-2xl mx-auto text-center">
    <span
      class="inline-block mb-4 px-3 py-1 rounded-full text-xs font-semibold bg-[var(--accent-lt)] text-[var(--accent)]"
      data-i18n="hero.badge"
    >{en.badge}</span>
    <h1
      class="text-4xl sm:text-5xl font-black text-[var(--txt-1)] mb-4 leading-tight"
      data-i18n="hero.headline"
    >{en.headline}</h1>
    <p
      class="text-lg text-[var(--txt-2)] mb-8"
    ...
    <p
      class="mt-3 text-xs text-[var(--txt-muted)]"
      data-i18n="hero.trust"
    >{en.trust}</p>
  </div>
</section>
```
...
<section class="py-16 px-4 bg-[var(--bg-dark)]">
  <div class="max-w-4xl mx-auto">
    <h2
      class="text-2xl font-bold text-white text-center mb-10"
      data-i18n="problem.heading"
    >{en.heading}</h2>
...
- [ ] **Step 3: Create `src/components/HowItWorks.astro`**
...
<section class="py-16 px-4 bg-[var(--bg-primary)]">
  <div class="max-w-3xl mx-auto">
    <h2
      class="text-2xl font-bold text-[var(--txt-1)] text-center mb-10"
      data-i18n="howItWorks.heading"
    >{en.heading}</h2>
...
- [ ] **Step 4: Add these sections to `src/pages/index.astro`**
...
`http://localhost:4321` — Hero → dark Problem section → HowItWorks visible. Language toggle switches all three sections. No horizontal overflow.

- [ ] **Step 6: Commit**

```bash
git add src/components/Hero.astro src/components/Problem.astro src/components/HowItWorks.astro src/pages/index.astro
git commit -m "feat: add Hero, Problem, HowItWorks sections"
```

---

## Task 8: Features, Screenshots, FAQ
...
- [ ] **Step 2: Create `src/components/Screenshots.astro`**

Placeholder divs stand in until real screenshots land in `public/screenshots/`. To swap in real images: replace the `<div class="w-[220px] ...">` with `<img src="/screenshots/overview.png" alt="Overview screen" class="w-[220px] h-[440px] object-cover rounded-3xl" />` etc.
...
<section class="py-16 px-4 bg-[var(--bg-alt)]">
  <div class="max-w-4xl mx-auto text-center">
    <h2
      class="text-2xl font-bold text-[var(--txt-1)] mb-10"
      data-i18n="screenshots.heading"
    >{en.heading}</h2>
      ...
      <div id="carousel-track" class="flex transition-transform duration-300">
        {slides.map(({ alt }) => (
          <div class="flex-shrink-0 w-full flex justify-center">
            <div class="w-[220px] h-[440px] bg-[var(--bg-dark)] rounded-3xl flex items-center justify-center text-white/30 text-sm">
              {alt}
            </div>
          </div>
        ))}
...
<script>
  ...
  const dots = document.querySelectorAll<HTMLElement>('#dots button');
  let current = 0;
...
<section class="py-16 px-4 bg-[var(--bg-primary)]">
  <div class="max-w-2xl mx-auto">
    <h2
      class="text-2xl font-bold text-[var(--txt-1)] text-center mb-8"
      data-i18n="faq.heading"
    >{en.heading}</h2>
...
`http://localhost:4321` — scroll through all six sections. Carousel prev/next/dots cycle slides correctly. FAQ items expand and collapse. Language toggle updates all text.

- [ ] **Step 6: Commit**

```bash
git add src/components/Features.astro src/components/Screenshots.astro src/components/Faq.astro src/pages/index.astro
git commit -m "feat: add Features, Screenshots, FAQ sections"
```
...
- [ ] **Step 1: Create `src/components/FooterCta.astro`**
...
<section class="py-16 px-4 bg-[var(--accent)] text-center">
  <h2
    ...
    data-i18n="footerCta.heading"
  >{en.heading}</h2>
  <p
    class="text-orange-100 mb-6 text-sm"
    data-i18n="footerCta.subline"
  >{en.subline}</p>
  <div class="flex justify-center">
...
<footer class="py-4 px-4 bg-[var(--bg-dark)] text-center">
  <p
    class="text-xs text-[var(--txt-muted)]"
    data-i18n="footer.copy"
  >{en.copy}</p>
</footer>
```

- [ ] **Step 3: Complete `src/pages/index.astro`** — replace the entire file:
...
Expected: `dist/` created, BUILD SUCCESS, 0 TypeScript errors.

- [ ] **Step 5: Commit**

```bash
git add src/components/FooterCta.astro src/components/Footer.astro src/pages/index.astro
git commit -m "feat: wire all sections in index.astro"
```
...
Open `http://localhost:4321`. Run through each item:

- [ ] All 9 sections visible, no browser console errors
- [ ] Navbar sticky on scroll
- [ ] Language toggle: click "ES" → all text switches to Spanish, button shows "EN"
- [ ] Refresh with ES active → Spanish preserved
- [ ] Switch back to EN → English restored
- [ ] Hero EmailForm: submit email only → success state + Supabase row with `whatsapp = null`
- [ ] FooterCta EmailForm: submit WhatsApp only (`+15551234567`) → success + row with `email = null`
- [ ] Submit both fields → success, both columns populated
- [ ] Submit empty → "Enter your email or WhatsApp number" (in active language)
...
- [ ] Submit duplicate email → success shown (23505 treated as success)
- [ ] News toggle off (default) → `subscribe_news = false`
- [ ] News toggle on → `subscribe_news = true`
- [ ] Carousel prev/next buttons cycle all 3 slides
- [ ] Carousel dot buttons jump to correct slide, active dot highlighted
- [ ] FAQ items expand and collapse natively
- [ ] Resize browser to 375px → single-column layout, no horizontal scroll
...
Open preview URL. Repeat key items: form submit, language toggle, carousel, build has no TS errors.

- [ ] **Final commit**

```bash
git add -A
git commit -m "chore: all sections complete, build and smoke test pass"
```
